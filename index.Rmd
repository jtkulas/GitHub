...
4. Advanced topics ([slides 22-43](#advanced))
...
???
You should be able to connect to any project that's *public* on GitHub, so search around a bit on [www.github.com](https://github.com/). You just won't be able to "push" ([discussed on a later slide](#pushpull)).

---
class: inverse

## .fancy[[Connecting to a GitHub project](https://www.youtube.com/watch?v=YxZ8J2rqhEM) Step I]
...
## .fancy[[Connecting to a GitHub project](https://www.youtube.com/watch?v=YxZ8J2rqhEM) Step II]
...
## .fancy[[Connecting to a GitHub project](https://www.youtube.com/watch?v=YxZ8J2rqhEM) Step III]
...
.pull-left[The previous slides should be sufficient to get a beginner connected to a project that's hosted on GitHub.  

The following slides present topics that an intermediate-level user may be interested in (e.g., someone who has already mastered setting up GitHub projects within `r icon_style(fontawesome("r-project"), fill="royalblue")` Studio). These "advanced" topics include:  

1. Inviting collaborators ([23-24](#invite))
  + Resolving conflicts ([25-26](#resolve))
2. Structuring (forking and branching) ([27-29](#fork))
3. Communicating with others
  + Issues ([30-32](#issue))
  + Notifications ([33-36](#notify))
  + Pull requests ([37-43](#pullrequest))
]
...
# .fancy[However...]`r emo::ji("boxing_glove")`

Take note here that if you try "pulling" prior to committing local changes you have made, you may encounter a .heatinline[**conflict**]

.pull-left[
![](https://media.giphy.com/media/lojil9HflxEjOq4gBz/giphy.gif)
]

.pull-right[.large[We do have a work-around for when this happens...]]

---

# .fancy[Resolving conflict(s)] `r emo::ji("sparkling_heart")`
...
.pull-left[ 
Take note here that any local work you've done will be obliviated... this work-around is therefore only useful when the information on the repository is reasonably considered to be *more* important or *more* up-to-date than what you have locally:
...
---
name: issue
## .fancy[Issues (whazzat?)]:

.pull-left[You can think of `issues` as computer-based post-it notes.  

You'll want to write and manage these directly through GitHub (instead of through `r icon_style(fontawesome("r-project"), fill="royalblue")` Studio), but they are helpful for keeping track of ideas and "things you want to work on" without being overly committal.
]
...
???
Issues can be more elaborate, and can also be [linked to pull requests](https://docs.github.com/en/issues/tracking-your-work-with-issues/linking-a-pull-request-to-an-issue#linking-a-pull-request-to-an-issue-using-a-keyword). We're focusing on the simplest of uses here in this tutorial. 

---
## .fancy[Issues (How to create)]:

1. Navigate to your online repository to [create an issue](https://docs.github.com/en/free-pro-team@latest/github/managing-your-work-on-github/creating-an-issue):
![](New Issue.png)
--
2. ...and then
[assign the issue](https://docs.github.com/en/free-pro-team@latest/github/collaborating-with-issues-and-pull-requests/creating-a-pull-request) to a collaborator, if you'd like to (this isn't necessary): 
![](Issue.png)

---
name: notify
class: inverse
## .fancy["Notifications"] `r emo::ji("envelope_with_arrow")`

It is possible to be notified (via e-mail) when somebody makes a push to a repository .saltinline[*that you "own"*]. This alerts you to the fact that a change has been made, and reminds you that you should "pull" prior to working on the project. To activate these notifications, navigate to the repository on GitHub, then enter your e-mail address... 
...
---
name: pullrequest
## .fancy[Pull request]: (I) `r emo::ji("magnet")`
...
.pull-right[
**IF** you do use different branches, then at some point you'll want to *integrate* something from a developmental branch onto the main branch. This is typically accomplished via a .heatinline[**pull request**]. 

A .heatinline[**pull request**] answers the question: .large["**What did you change?**"]
]

???
Pull requests are also used when you want to contribute to a repository where you're not a formal owner or collaborator - this is very popular with "open source" projects. See, for example, the [ethereum](https://github.com/ethereum) network of repositories.

---

## .fancy[Pull request]: (II) 

Going through a formal .heatinline[**pull request**] when considering changes among team members places a review frame on the changes. Now you'll be able to share, comment, and receive feedback on your contribution before it's integrated (aka `merged`).

![](review.jpg)

---

## .fancy[Pull request]: (III)

Just like `issues`, you'll want to initiate your pull request from within the repository website [hosted on GitHub](https://docs.github.com/en/free-pro-team@latest/github/collaborating-with-issues-and-pull-requests/creating-a-pull-request):

![](pull1.PNG)

---

## .fancy[Pull request]: (IV)

.pull-left[.large[First commit your proposed additions to the development branch (within `r icon_style(fontawesome("r-project"), fill="royalblue")` Studio), and then follow the `New Pull Request` wizard options]
...
.right-column[
There will be an option of "which branch to `pull` to" - this will be the primary project branch (typically named `main`):  
<br />
...
---
## .fancy[Pull request]: (Viola! `r emo::ji("violin")`)

There is a final option of submitting a "full pull request" or a mere draft. Go ahead and submit the `Create Pull Request` (you don't need no stinkin' draft)!
